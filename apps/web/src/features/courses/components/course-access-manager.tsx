...
export function CourseAccessManager({
  ...
  const [attachingStudentId, setAttachingStudentId] = useState<string | null>(
    null,
  );
  ...
  const [createStudentForm, setCreateStudentForm] = useState<CreateStudentForm>(
    DEFAULT_CREATE_STUDENT_FORM,
  );
  const [searchStudentForm, setSearchStudentForm] = useState<SearchStudentForm>(
    DEFAULT_SEARCH_STUDENT_FORM,
  );
  const [searchResults, setSearchResults] = useState<
    TeacherStudentSearchResult[]
  >([]);
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Не удалось загрузить данные по доступу",
      );
    } finally {
      setLoading(false);
    }
  ...
  const searchStudents = async () => {
    ...
    try {
      const results = await enrollmentsApi.searchCourseStudents(
        accessToken,
        courseId,
        {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Не удалось выполнить поиск студентов",
      );
      setSearchPerformed(true);
    } finally {
  ...
  const enrollExistingStudent = async (student: TeacherStudentSearchResult) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось зачислить студента",
      );
    } finally {
  ...
  const createStudent = async () => {
    ...
    try {
      const response = await enrollmentsApi.createStudentForCourse(
        accessToken,
        courseId,
        {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось создать студента",
      );
    } finally {
  ...
  const deactivateInvitation = async (invitationId: string) => {
    ...
    try {
      const updated = await invitationsApi.deactivate(
        accessToken,
        invitationId,
      );
      setInvitations((prev) =>
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Не удалось деактивировать приглашение",
      );
    } finally {
      setBusyId(null);
    }
  ...
  const removeStudent = async (enrollment: CourseEnrollment) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось удалить студента",
      );
    } finally {
      setBusyId(null);
    }
  ...
  return (
    <>
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="grid gap-6">
          <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Найдите существующего студента по имени или группе и зачислите в
              этот курс.
            </p>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button
                ...
                disabled={searchingStudents || loading}
              >
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Если студента ещё нет в системе, создайте ему аккаунт и сразу
              зачислите в курс.
            </p>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button
                ...
                disabled={!canCreateStudent || creatingStudent}
              >
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge
                          tone={invitation.isActive ? "success" : "warning"}
                        >
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      {invitation.isActive ? (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            void deactivateInvitation(invitation.id)
                          }
        ...
        <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            {loading ? (
              ...
              <p className="text-sm text-gray-500">
                Пока нет зачисленных студентов.
              </p>
            ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {enrollment.student?.email ?? "Email не указан"}
                      </p>
                      <p className="text-xs text-gray-500">
                        Группа:{" "}
                        {enrollment.student?.group?.trim() || "не указана"}
                      </p>
        ...
        {error ? (
          <p className="text-sm text-red-500 lg:col-span-2">{error}</p>
        ) : null}
        {notice ? (
          <p className="text-sm text-emerald-500 lg:col-span-2">{notice}</p>
        ) : null}
      </div>
