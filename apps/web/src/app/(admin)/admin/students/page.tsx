...
export default function AdminStudentsPage() {
  ...
  const saveStudent = async () => {
    ...
    if (editState.fullName.trim().length < 2 || editState.email.trim().length < 5) {
      setError('Проверьте корректность ФИО и email студента');
      return;
    }
  ...
  return (
    <>
      <div className="grid gap-6">
        ...
        <Card>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-4">
            ...
            <Input
              ...
              onChange={(event) => setCreateState((prev) => ({ ...prev, group: event.target.value }))}
              maxLength={64}
        ...
        {!loading && sortedStudents.length === 0 ? (
          <EmptyState
            ...
            description="Создайте первого студента через форму выше или импортируйте CSV-файл."
          />
        ) : null}
      ...
      <StudentImportModal
        ...
        onImported={async () => {
          await loadStudents();
          setNotice('Импорт студентов успешно завершён');
        }}
      />
    </>
  );
