...
export default function EditCoursePage({ params }: EditCoursePageProps) {
  ...
  const loadCourse = useCallback(async () => {
    ...
    try {
      const data = await coursesApi.getMyTeacherCourseById(
        accessToken,
        params.courseId,
      );
      setCourse(data);
  ...
  const deleteCourse = async () => {
    ...
    const confirmed = window.confirm(
      "Удалить курс? Это действие нельзя отменить.",
    );
    if (!confirmed) return;
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        title={
          course ? `Редактирование: ${course.title}` : "Редактирование курса"
        }
        ...
        <>
          ...
          <Card>
            <h2 className="text-lg font-semibold text-gray-700">
              Приглашения и студенты
            </h2>
            <p className="mt-1 text-sm text-gray-500">
              Добавляйте студентов в курс вручную, выбирайте из списка или
              импортируйте их из файла.
            </p>
            <div className="mt-4">
              <CourseAccessManager
                courseId={course.id}
...
function LoadingBlock() {
  return (
    <div className="h-[420px] animate-pulse rounded-2xl border border-gray-200 bg-gray-50" />
  );
}
